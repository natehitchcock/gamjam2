...
export default class Controller implements IComponent {
    ...
    shouldBeDodging() {
        return (this.dodgeStartTimer !== undefined
            && this.data.dodgeTime !== undefined
            && (this.myClock.getElapsedTime() - this.dodgeStartTimer) <= this.data.dodgeTime);
    }
    ...
    dodgeOwner(desiredMove: THREE.Vector2) {
        if(this.dodgeDirection === undefined) {
            ...
            this.dodgeStartPosition = new THREE.Vector2(this.owner.position.x, this.owner.position.y);
        }

        const dodgeSpeed = this.data.dodgeDistance / this.data.dodgeTime;
        const dodgeElapsedTime = this.myClock.getElapsedTime() - this.dodgeStartTimer;

        const targetPosition = this.dodgeDirection.clone()
        .normalize()
        ...
        if(this.data.dodgeTime - dodgeElapsedTime <= 0.03) {
            ...
            this.owner.sharedData.nextMove = new THREE.Vector3(targetPosition.x, targetPosition.y, 0);
        }
    }
    ...
    lookOwner(desiredLook: THREE.Vector2, dt: number) {
        ...
        this.owner.sharedData.lastLook = this.owner.sharedData.look ? this.owner.sharedData.look.clone() : undefined;
