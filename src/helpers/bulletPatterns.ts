...
export default class BulletPatterns {
    public static StraightShot(me: Entity, target: Entity, shotIndex: number, spreadDeg: number, bulletSpeed: number) {
    ...
    public static FiveScatterBurstShot(
        me: Entity,
        target: Entity,
        shotIndex: number,
        spreadDeg: number,
        ...
        const dirs = radAngles.map(angle =>
            new THREE.Vector3(Math.cos(angle), Math.sin(angle))
